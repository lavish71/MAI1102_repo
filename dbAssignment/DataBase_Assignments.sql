...
SELECT c1.horsepower, c1.id, c2.id
FROM cars_data c1 INNER JOIN cars_data c2 
ON c1.id = c2.id
AND c1.id > c2.id
...
SELECT AVG(some) 
from (
	SELECT COUNT(flight) AS Total 
    FROM cars_data 
    GROUP BY airline) car_table; 
